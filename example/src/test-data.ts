import { FlameNode, WaterfallIntervals } from '../../src/index';
...
type Layer = { rest: number; flameNodes: FlameNode[] };
...
const generateRandomLevel = (count: number, minChild = 1, maxChild = 10): Layer => {
    ...
    const items = Array(childrenCount)
        .fill(null)
        .map((): FlameNode => ({ children: [], duration: 0, name: '', start: 0 }));
...
const generateRandomNesting = (count: number, minChild: number, maxChild: number) => {
    ...
    while (rest > 0 && !isStopped) {
        if (!levels.length) {
            ...
            rootFlameNodes = layer.flameNodes;
            rest = layer.rest;
        } else {
            const level: FlameNode[][] = levels[levels.length - 1];
            const innerLevel: FlameNode[][] = [];
...
const flattenFlameNodeAndMap = (
    ...
    cb: (items: FlameNode[], parent: FlameNode | undefined) => FlameNode[],
    parent?: FlameNode | undefined
) => {
...
export const generateRandomTree = ({
    ...
    const mappedNestingArrays = flattenFlameNodeAndMap(
        rootNodes.root,
        (items: FlameNode[], parent: FlameNode | undefined): FlameNode[] => {
